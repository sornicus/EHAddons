...
local ZoneEntryCached = {}; --- [ NpcID ] = true if mob was already cached when entering zone
--- Begins searching for an NPC.
-- @return True if successfully added.
local function ScanAdd ( NpcID )
	local Name = me.TestID( NpcID );
	if ( Name ) then -- Already cached on zone entry; scan but don't alert
		ZoneEntryCached[ NpcID ] = true;
		CacheList[ NpcID ] = Name;
	end
	-- Always add to scan pool so we alert if mob appears while in zone
...
do
	...
	local function OnFound ( NpcID, Name )
		-- Suppress further alerts for this mob until next zone entry
		ZoneEntryCached[ NpcID ] = true;
	...
	function me.Frame:OnUpdate ( Elapsed )
		...
		if ( NextUpdate <= 0 ) then
			...
			for NpcID in pairs( ScanIDs ) do
				if ( not ZoneEntryCached[ NpcID ] ) then
					local Name = me.TestID( NpcID );
					if ( Name ) then
						OnFound( NpcID, Name );
					end
...
do
	...
	function me.Frame:PLAYER_ENTERING_WORLD ()
		...
		-- Reset per-zone cache tracking so all mobs can alert again in the new zone
		wipe( ZoneEntryCached );
