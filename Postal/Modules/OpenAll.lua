	...
	if mailIndex > 0 then
		...
		if wait then
			...
			elseif lastNumAttach ~= attachCount then
				...
				if waitRetries >= 10 then
					-- Server hasn't responded after retries, force advance
					Postal:Print("Postal: server timeout on mail "..mailIndex..", skipping.")
					wait = false
					waitRetries = 0
					mailIndex = mailIndex - 1
					attachIndex = ATTACHMENTS_MAX_RECEIVE
		...
		if mailType == "NonAHMail" then
			...
			if not strfind(sender, factionLocale) and not strfind(sender, factionEnglish) then
